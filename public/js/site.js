jQuery(document).ready(function($) {
    $('.catalog-item-menu').hover(
        function(){
            $(this).find('.collapse_menu').removeClass('d-none').show();
            $(this).find('i').removeClass('fa-plus').addClass('fa-minus');
        },
        function(){
            $(this).find('.collapse_menu').addClass('d-none').hide();
     ...
     */
    $('.send-call').submit(function (e) {
        // Получение данных формы
        e.preventDefault();
        var formData = new FormData();
        var url= '/ru/basket/callRequest/';
        document.getElementById('fname').value;
        formData.append('fname',  document.getElementById('fname').value);
        formData.append('phoneNumber',  document.getElementById('phoneNumber').value);

        // Пример отправки данных формы на сервер (используйте свой метод)
        $.ajax({
            url: url,
            data: formData,
            processData: false,
          ...
          beforeSend: function () {
                ...
                $('#requestCallForm').append(spinner);
              console.log('CSRF Token:', $('meta[name="csrf-token"]').attr('content'));
            },
            success: function(html) {
                $('#requestCallForm').find('.spinner-border').remove();
            }
        });
        $('#exampleModal').modal('hide');
    });
   // $('#catalog-sidebar > ul ul').show();
    ...
    $('form.request-to-call').submit(function (e) {
        // отменяем отправку формы стандартным способом
        e.preventDefault();
        // получаем данные этой формы добавления в корзину
        var $form = $(this);
